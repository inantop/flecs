...
void Map_get_from_null() {
    test_assert(ecs_map_get(NULL, int, 0) == NULL);
}
...
void Map_copy() {
    ...
    ecs_map_t *copy = ecs_map_copy(map);
    test_int(ecs_map_count(copy), 4);
...
void Map_memory() {
    ...
    ecs_map_memory(map, &allocd, &used);
    test_assert(allocd != 0);
    test_assert(used != 0);
